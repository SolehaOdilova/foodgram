...
def build_shopping_list(user):
    shopping_data = (
        ...
        .filter(recipe__in=user.shopping_cart_recipes.values_list('id', flat=True))
        .values('ingredient')
        .annotate(total=Sum('amount'))
    )

    ingredient_map = Ingredient.objects.in_bulk([item['ingredient'] for item in shopping_data])
