...
class SubscriptionManageMixin:
    ...
    add_serializer = None    # Определяется во ViewSet
    ...
    def add_and_delete(self, id, relation_name):
        ...
        try:
            ...
            if self.request.method == 'POST':
                ...
                relation.add(target)
                serializer = self.add_serializer(target, context={'request': self.request})
            ...
            return Response({'errors': 'Метод не поддерживается.'}, status=status.HTTP_405_METHOD_NOT_ALLOWED)

        except Exception as e:
            return Response({'errors': f'Внутренняя ошибка: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)