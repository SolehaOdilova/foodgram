...
class RecipesViewSet(ModelViewSet, RelationToggleMixin):
    ...
    def get_queryset(self):
        ...
        if user.is_authenticated:
            return Recipe.objects.with_user_flags(user).select_related('author').prefetch_related('tags')
        return base
    ...
    def create(self, request, *args, **kwargs):
        ...
        full_data = RecipeDetailSerializer(recipe, context=self.get_serializer_context())
        return Response(full_data.data, status=status.HTTP_201_CREATED)

    @action(methods=['get', 'post', 'delete'], detail=True, url_path='favorite')
    def favorites(self, request, pk=None):
        return self.toggle_relation(pk, 'favorited_recipes')

    @action(methods=['get', 'post', 'delete'], detail=True, url_path='shopping_cart')
